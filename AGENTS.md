...
### `bathhouse_scrub` — 搓澡（快速回血）

请求王师傅搓澡。需要先移动到王师傅附近（120 像素内），然后调用此工具。搓澡持续 8 秒，每 0.5 秒回复 10 HP。
...
```
🧖 王师傅开始给你搓澡了！「放松肩膀，呼吸慢一点。」
⏱ 持续 8 秒 | 💚 每 0.5 秒回复 10 HP
```
...
> **提示：** 王师傅位于搓澡区附近 (410, 110)。先用 `bathhouse_move` 走到他旁边，再调用 `bathhouse_scrub`。
